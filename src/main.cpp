...
void handleVideo(librtmp::RTMPMediaMessage m, librtmp::ClientParameters* sourceParams) {
    ...
    const char* fType = (m.video.d.frame_type == 1) ? "Keyframe" : "Interframe"; // 2 = interframe
    const char* pType = (m.video.d.avc_packet_type == 0) ? "AVCC-Hdr" : "AVCC-Access-Unit"; //

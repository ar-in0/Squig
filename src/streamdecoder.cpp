...
StreamDecoder::StreamDecoder(librtmp::RTMPMediaMessage m, librtmp::ClientParameters sourceParams)
...
void StreamDecoder::registerAVCCExtraData() {
    ...
    m_pDecCtx->extradata = (uint8_t* )av_mallocz(eData.size() + AV_INPUT_BUFFER_PADDING_SIZE);
...
void StreamDecoder::h264AUDecode(uint8_t* pAUData, size_t payloadSize, uint64_t dTime, uint32_t cTime) {
...
void StreamDecoder::process(librtmp::RTMPMediaMessage& m) {
    ...
    cv::Mat img(m_pFrameBGR->height, m_pFrameBGR->width, CV_8UC3, m_pFrameBGR->data[0], m_pFrameBGR->linesize[0]);
